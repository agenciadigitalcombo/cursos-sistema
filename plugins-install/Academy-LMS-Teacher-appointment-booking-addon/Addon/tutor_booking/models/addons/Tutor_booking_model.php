...
class Tutor_booking_model extends CI_Model
{
    ...
    public function add_schedule()
    {
        ...
        $data['title']=htmlspecialchars($info_from_view['title']);
        $data['category_id']=htmlspecialchars($info_from_view['category_id']);
        $data['sub_category_id']=htmlspecialchars($info_from_view['sub_category_id']);
        $data['price_type']=htmlspecialchars($info_from_view['price_type']);
        $data['price']=htmlspecialchars($info_from_view['price']);
        $data['tutor_id']= $this->session->userdata('user_id');
        $data['date_added']=strtotime(Date('Y-m-d\TH:i'));
        $data['tution_class_type']=htmlspecialchars($info_from_view['class_type']);
    ...
    public function edit_schedule()
    {
        ...
        $data['tutor_id']= $this->session->userdata('user_id');
        $data['tution_class_type']=htmlspecialchars($info_from_view['class_type']);
        $booking_id=htmlspecialchars($info_from_view['booking_id']);
        $this->db->where('id', $booking_id);
        $this->db->update('tutor_booking', $data);

       $schedule_id_in_Array=htmlspecialchars($info_from_view['schedule_ids']);

       $total_no_schedule=sizeof($info_from_view['start_time'])-1;
       $single_schedule;
       $single_schedule['booking_id']=htmlspecialchars($info_from_view['booking_id']);
       $single_schedule['tutor_id']=htmlspecialchars($info_from_view['tutor_id']);
       $not_in_array=array();
         ...
         for($i=0; $i<$total_no_schedule; $i++)
         {
            ...
            {
                $tution_type=$m."_tution_type_indentify";
                $single_schedule['tuition_type']=htmlspecialchars($info_from_view[$tution_type]);
                $single_schedule['tution_class_type']=$data['tution_class_type'];
                $single_schedule['start_time']=strtotime($info_from_view['start_time'][$i]);
                $duration=(int)$info_from_view['end_time'][$i];
                $single_schedule['end_time']=strtotime(" +$duration minutes",strtotime($info_from_view['start_time'][$i]));
    ...
    {
        $data['start_time']=htmlspecialchars($this->input->post('start_time'));
        $data['end_time']=htmlspecialchars($this->input->post('end_time'));
    ...
    public function get_all_schedules_after_filter($filter=array(),$limit,$offset,$action)
    {
      ...
      $searched_data['price_range_style_left']=$filter['price_range_style_left'];
      $searched_data['price_range_style_right']=$filter['price_range_style_right'];
    ...
    public function total_student($tutor_id)
    {
        ...
        $tutor_schedule= $this->db->get('tutor_schedule');
        $total_student=$tutor_schedule->num_rows();
    ...
    public function update_live_class_settings() {
        ...
        $data['zoom_api_key'] = htmlspecialchars($this->input->post('zoom_api_key'));
        $data['zoom_secret_key'] = htmlspecialchars($this->input->post('zoom_secret_key'));
