...
class Affiliate_course_model extends CI_Model
{
    ...
    public function post_affiliator_application()
    {
        ...
        $user_details = $this->user_model->get_all_user(html_escape($this->input->post('id')))->row_array();

        // CHECK IF THE PROVIDED ID AND EMAIL ARE COMING FROM VALID USER
        if ($user_details['email'] == html_escape($this->input->post('email'))) {
            ...
            $data['user_id'] = html_escape($this->input->post('id'));
